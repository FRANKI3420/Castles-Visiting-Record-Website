...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 💡 追加：ページ読み込み時に、現在選ばれているCSVでリストを自動更新する
    const regionSelect = document.getElementById("region");
    if (regionSelect.value) {
        // changeイベントを手動で発生させ、CSV読み込みとフィルタリングを走らせる
...
function updateCastlePicker() {
    ...
    fetch(selectedcsv)
        ...
        .then(csvData => {
            ...
            const rawData = localStorage.getItem("storedData4"); // ここを実際に使っているキーに変えてください
...
document.getElementById("region").addEventListener("change", function () {
    ...
    fetch(selectedcsv)
        ...
        .then(csvData => {
            ...
            // 💡 ここが重要：最新の保存データをこのタイミングで読み出す
            const storedData = JSON.parse(localStorage.getItem("storedData4")) || [];
            // 保存されている城名（またはID）のリストを作成
            const visitedNames = storedData.map(data => data.castleName);

            citySelect.innerHTML = '<option value="">未選択</option>';

            castleLocations2.forEach(function (castle) {
                // 💡 保存済みリストに含まれていない場合のみ、選択肢に追加
            ...
            if (citySelect.options.length === 1 && this.value !== "未選択") {
                const option = document.createElement("option");
                option.textContent = "すべて取得済みです！🎉";
...
async function displayStoredData() {
    ...
    let totalCount = 0;
    try {
        const response = await fetch('data/catsle_card_zoku.csv');
...
function getAllCastleIds() {
    // ローカルストレージからデータを取得（続・城カード用 storedData4）
    const storedData = JSON.parse(localStorage.getItem("storedData4")) || [];
...
function getRcordList() {
    ...
    // 続・城カード用データ（storedData4）を取得
    let storedData = JSON.parse(localStorage.getItem("storedData4")) || [];

    // --- ID順（昇順）に並べ替え ---
    storedData.sort((a, b) => Number(a.castleId) - Number(b.castleId));

    // ヘッダーを追加（続カード版用のクラス zoku-card-header を使用）
    let html = `
        <div class="list-header delete-header zoku-card-header">
            <h3>削除する城カードの選択（続100名城）</h3>
    ...
    storedData.forEach(record => {
        html += `
            <div class="castle-item delete-item zoku-card-item">
                <input type="checkbox" id="check-card4-${record.castleId}" value="${record.castleName}" class="delete-checkbox">
                <label for="check-card4-${record.castleId}" class="delete-label">
